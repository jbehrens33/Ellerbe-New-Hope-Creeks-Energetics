...
model_bayes_nhc <- function(sm_input_dir = 'data/sm_input/',
  ...
  for(i in 1:length(site_years$site)){
    ...
    tryCatch({
      ...
      K600_mod <- readr::read_csv('data/model_runs/MLE/dat_fit/{site}_2021-01-01_2021-12-31_dat_fit.csv') %>% 
        pull(K.600.daily)
      
      ice_ts <- # read in daily time series of ice days
        
        # merge these two things, and replace K600.daily with 0 where ice is present
        K600_ice <- # merge
        
        # another if statement to declare which ice time series you're using
        ice_ts_use <- K600_ice$ # source of p(ice); either camera, satelite, or temp
        
        # add additional code to shrink K600_lnQ_nodes_sdlog to something small
        # to force K to be small on days with lots of ice
        ...
        bayes_specs <- streamMetabolizer::specs(
          ...
          K600_lnQ_nodes_meanlog = rep(log(ice_ts_use, length(nodes))),  # default; see ?specs
          K600_lnQ_nodes_sdlog = c(rep(0.7, 8)),
          burnin_steps = 1000,
          saved_steps = 500)
    },
